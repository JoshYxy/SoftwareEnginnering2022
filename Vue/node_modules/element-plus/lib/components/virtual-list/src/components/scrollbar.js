...
const ScrollBar = vue.defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const trackStyle = vue.computed(() => ({
      position: "absolute",
      width: defaults.HORIZONTAL === props.layout ? `${trackSize.value}px` : "6px",
      height: defaults.HORIZONTAL === props.layout ? "6px" : `${trackSize.value}px`,
    ...
    const attachEvents = () => {
      event.on(window, "mousemove", onMouseMove);
      event.on(window, "mouseup", onMouseUp);
      const thumbEl = vue.unref(thumbRef);
      if (!thumbEl)
        return;
      onselectstartStore = document.onselectstart;
      document.onselectstart = () => false;
      event.on(thumbEl, "touchmove", onMouseMove);
      event.on(thumbEl, "touchend", onMouseUp);
    };
    const detachEvents = () => {
      event.off(window, "mousemove", onMouseMove);
      event.off(window, "mouseup", onMouseUp);
      document.onselectstart = onselectstartStore;
      onselectstartStore = null;
      const thumbEl = vue.unref(thumbRef);
      if (!thumbEl)
        return;
      event.off(thumbEl, "touchmove", onMouseMove);
      event.off(thumbEl, "touchend", onMouseUp);
    };
    ...
    vue.onMounted(() => {
      if (!core.isClient)
        return;
      event.on(trackRef.value, "touchstart", onScrollbarTouchStart);
      event.on(thumbRef.value, "touchstart", onThumbMouseDown);
    });
    vue.onBeforeUnmount(() => {
      event.off(trackRef.value, "touchstart", onScrollbarTouchStart);
      detachEvents();
    });
    return () => {
      return vue.h("div", {
