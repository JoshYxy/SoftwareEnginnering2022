...
const addStyle = async (options, parent, instance) => {
  ...
  if (options.fullscreen) {
    ...
    for (const property of ["top", "left"]) {
      ...
      maskStyle[property] = `${options.target.getBoundingClientRect()[property] + document.body[scroll] + document.documentElement[scroll] - parseInt(style.getStyle(document.body, `margin-${property}`), 10)}px`;
    }
...
const addClassList = (options, parent, instance) => {
  if (instance.originalPosition.value !== "absolute" && instance.originalPosition.value !== "fixed") {
    style.addClass(parent, "el-loading-parent--relative");
  } else {
    style.removeClass(parent, "el-loading-parent--relative");
  }
  if (options.fullscreen && options.lock) {
    style.addClass(parent, "el-loading-parent--hidden");
  } else {
    style.removeClass(parent, "el-loading-parent--hidden");
  }
};
